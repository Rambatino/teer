...
class Engine
  ...
  def parse_template(template)
    ...
    template.each do |k, v|
      ...
      if k == 'text' && (!v.nil? && v.length > 0)
        new_text = interpolate.call(v[@locale.to_s])
    ...
    @pre_parsed_finding = CGI.unescapeHTML(pre_parsed_text[0...-1]) if (!pre_parsed_text.nil? && pre_parsed_text.length > 0)
    CGI.unescapeHTML(text[0...-1]) if (!text.nil? && text.length > 0)
  end
end
